 ...
 * searching the digits file which is encoded packing two decimal digits into
 * each byte.
 */
var halfByteInterface = (function() {
  ...
  halfByteInterface.prototype.matchingBytes = function(position, query, oddLength) {
    ...
    var start_position = (position >> 1) - this.bufferPosition,
        i = 0;

    while(this.buffer[start_position + i] === query[i] && query[i]) {
      i++;
    }
  ...
  halfByteInterface.prototype.onReadyAt = function(position, len, cb, final_cb) {
    ...
    var me, waitLoop;
    if ((position >> 1) + len < this.bufferPosition + this.buffer.length) {
...
/*
 * The searchPiFor function is the API for the search process
 */
exports.searchPiFor = function(query, indexDir, digitsFile, cb) {
  var firstDigitIndex, digitsInterface, i, startPosition, evenQuery, oddQuery,
    evenQueryOddLen, oddQueryOddLen;

  var asyncSearch = function(cb) {
    ...
    if (firstDigitIndex.cursor < firstDigitIndex.buffer.length) {
      ...
      while ( ( (startPosition = (startPosition || firstDigitIndex.step())) / 2 + evenQuery.length ) <= current_digit_buffer_end ) {
        ...
        if (firstDigitIndex.cursor >= firstDigitIndex.buffer.length) {
          ...
          return firstDigitIndex.onReady(asyncSearch, cb);
        }
      }

      // reaching here implies that digitsInterface needs rebuffering
      return digitsInterface.onReadyAt(startPosition, evenQuery.length, asyncSearch, cb);
    }
  ...
  // Pass asyncSearch as a callback for when digitsInterface is buffered
  return digitsInterface.onReadyAt(null, evenQuery.length, asyncSearch, cb);
};
